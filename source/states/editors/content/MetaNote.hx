...
class EventMetaNote extends MetaNote
{
	...
	function loadIcon()
	{
		if(events.length>1)
			return;
		else if(Paths.fileExists('images/editors/chartEditor/events/${events[0][0]}.png',IMAGE)){
			loadGraphic(Paths.image('editors/chartEditor/events/${events[0][0]}'));
		}
		else loadGraphic(Paths.image('editors/chartEditor/eventIcon'));
		setGraphicSize(ChartingState.GRID_SIZE);
		updateHitbox();
	}
