package funkin.stages.data.weeks.week6;

import funkin.stages.data.weeks.week6.objects.BackgroundGirls;
import funkin.states.GameOverState;
import funkin.data.dialogue.DialogueBox;
...
class SchoolEvil extends BaseStage
{
	override function create()
	{
		...
		if(_song.gameOverSound == null || _song.gameOverSound.trim().length < 1) GameOverState.deathSoundName = 'fnf_loss_sfx-pixel';
		if(_song.gameOverLoop == null || _song.gameOverLoop.trim().length < 1) GameOverState.loopSoundName = 'gameOver-pixel';
		if(_song.gameOverEnd == null || _song.gameOverEnd.trim().length < 1) GameOverState.endSoundName = 'gameOverEnd-pixel';
		if(_song.gameOverChar == null || _song.gameOverChar.trim().length < 1) GameOverState.characterName = 'bf-pixel-dead';
