...
import funkin.stages.objects.SpraycanAtlasSprite;
...
class PhillyStreets extends BaseStage
{
	...
	override function create()
	{
		...
		if (_song.gameOverLoop == null || _song.gameOverLoop.trim().length < 1)
			GameOverState.loopSoundName = 'gameOver-pico';
		if (_song.gameOverEnd == null || _song.gameOverEnd.trim().length < 1)
			GameOverState.endSoundName = 'gameOverEnd-pico';
		if (_song.gameOverChar == null || _song.gameOverChar.trim().length < 1)
			GameOverState.characterName = 'pico-dead';
		setDefaultGF('nene');
		gfGroup.y += 200;
		gfGroup.x += 50;
	...
	override function noteMiss(note:Note)
	{
		switch (note.noteType)
		{
			case 'weekend-1-firegun':
				...
				{
					GameOverState.deathSoundName = 'fnf_loss_sfx-pico-explode';
					GameOverState.loopSoundName = 'gameOverStart-pico-explode';
					GameOverState.characterName = 'pico-explosion-dead';
				}
		}
	}
