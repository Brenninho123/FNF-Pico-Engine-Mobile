	...
	function openGodotChartToPico()
	{
		fileDialog.open('song.json', 'Open Godot Chart JSON', null, function()
		{
			try
			{
				...
				fileDialog.save(Paths.formatToSongPath(getFileBase(filePath)) + '-pico.json', converted, function()
				{
		...
		fileDialog.open('chart.json', 'Open Codename chart.json', null, function()
		{
			...
			fileDialog.open('meta.json', 'Open Codename meta.json', null, function()
			{
				try
				{
					...
					fileDialog.save(saveName + '-pico.json', PsychJsonPrinter.print(convertedSong, ['sectionNotes', 'events']), function()
					{
		...
		fileDialog.openDirectory('Open Godot Charts Folder', function()
		{
			...
			try
			{
				...
				for (file in FileSystem.readDirectory(folder))
				{
					...
					try
					{
						File.saveContent(outFolder + '/' + Paths.formatToSongPath(baseName) + '-pico.json', convertGodotChart(File.getContent(fullPath), baseName));
						convertedCount++;
					}
					catch(e:Dynamic)
					{
	...
	function openCharacter(source:String)
	{
		...
		fileDialog.open(defaultName, 'Open ${sourceLabel(source)} Character', characterFilter(source), function()
		{
			try
			{
				...
				var saveName = Paths.formatToSongPath(getFileBase(sourcePath)) + '-pico.json';
		...
		fileDialog.openDirectory('Open ${sourceLabel(source)} Characters Folder', function()
		{
			...
			try
			{
				...
				for (file in FileSystem.readDirectory(folder))
				{
					...
					var baseName = getFileBase(file);
					File.saveContent(outFolder + '/' + Paths.formatToSongPath(baseName) + '-pico.json', convertCharacter(File.getContent(fullPath), baseName, source));
					convertedCount++;
				}
