...
import funkin.states.data.menus.story.MenuItem;
...
class StoryModeState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(WeekData.weeksList.length < 1)
		{
			...
			MusicBeatState.switchState(new funkin.utils.states.ErrorState("NO LEVELS ADDED FOR STORY MODE\n\nPress ACCEPT to go to the Week Editor Menu.\nPress BACK to return to Main Menu.",
				function() MusicBeatState.switchState(new funkin.data.editors.WeekEditorState()),
				function() MusicBeatState.switchState(new funkin.states.data.menus.MainMenuState())));
			return;
		}
	...
	function selectWeek()
	{
		if (!weekIsLocked(loadedWeeks[curWeek].fileName))
		{
			...
			try
			{
				...
				PlayState.loadSongWithVariationFallback(PlayState.storyPlaylist[0], null, curDifficulty);
			...
			new FlxTimer().start(1, function(tmr:FlxTimer)
			{
				...
				funkin.states.data.menus.freeplay.FreeplayState.destroyFreeplayVocals();
			});
