...
class FunkinLua
{
	...
	public function new(scriptName:String) {
		...
		@:privateAccess
		{
			...
			set('inGameOver', GameOverState.instance != null);
		...
		Lua_helper.add_callback(lua, "reloadState", function(?skipTransition:Bool = false) {
			if(game != null)
			{
				game.paused = true;
			...
			MusicBeatState.reloadState(skipTransition);
		});
		Lua_helper.add_callback(lua, "reloadCurrentState", function(?skipTransition:Bool = false) {
			if(game != null)
			{
				game.paused = true;
				game.canResync = false;
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, ?inFront:Bool = false) {
			...
			else
			{
				...
				GameOverState.instance.insert(GameOverState.instance.members.indexOf(GameOverState.instance.boyfriend), mySprite);
			}
		});
