...
class StageEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function normalizeStageJsonData()
	{
		if(stageJson == null) return;

		stageJson.defaultZoom = stageEditorFloat(stageJson.defaultZoom, 0.9);
		stageJson.camera_speed = stageJson.camera_speed != null ? stageEditorFloat(stageJson.camera_speed, 1) : 1;
		stageJson.boyfriend = normalizeDynamicPoint(stageJson.boyfriend, [770, 100]);
		stageJson.girlfriend = normalizeDynamicPoint(stageJson.girlfriend, [400, 130]);
		stageJson.opponent = normalizeDynamicPoint(stageJson.opponent, [100, 100]);
		stageJson.camera_boyfriend = normalizeFloatPoint(stageJson.camera_boyfriend, [0, 0]);
		stageJson.camera_opponent = normalizeFloatPoint(stageJson.camera_opponent, [0, 0]);
		stageJson.camera_girlfriend = normalizeFloatPoint(stageJson.camera_girlfriend, [0, 0]);

		if(stageJson.objects != null)
		{
			for (object in stageJson.objects)
				normalizeStageObjectData(object);
		}
	}

	static function normalizeStageObjectData(object:Dynamic)
	{
		...
		for (field in ['x', 'y', 'alpha', 'angle'])
		{
			...
			if(value != null) Reflect.setField(object, field, stageEditorFloat(value, field == 'alpha' ? 1 : 0));
		}

		var scale:Dynamic = Reflect.field(object, 'scale');
		if(scale != null) Reflect.setField(object, 'scale', normalizeFloatPoint(scale, [1, 1]));

		var scroll:Dynamic = Reflect.field(object, 'scroll');
		if(scroll != null) Reflect.setField(object, 'scroll', normalizeFloatPoint(scroll, [1, 1]));

		var filters:Dynamic = Reflect.field(object, 'filters');
		if(filters != null)
		{
			var parsed = Std.parseFloat(Std.string(filters));
			if(!Math.isNaN(parsed)) Reflect.setField(object, 'filters', Std.int(parsed));
		}
	}
	...
	static function normalizeFloatPoint(value:Dynamic, fallback:Array<Float>):Array<Float>
	{
		...
		if(Std.isOfType(value, Array))
		{
			...
			return [
				array.length > 0 ? stageEditorFloat(array[0], fallback[0]) : fallback[0],
				array.length > 1 ? stageEditorFloat(array[1], fallback[1]) : fallback[1]
			];
		}

		var text = Std.string(value).replace(';', ',').replace('|', ',');
		var split = text.contains(',') ? text.split(',') : text.split(' ');
		if(split.length > 1)
			return [stageEditorFloat(split[0], fallback[0]), stageEditorFloat(split[1], fallback[1])];

		return fallback.copy();
	}

	static function stageEditorFloat(value:Dynamic, fallback:Float):Float
	{
	...
	function addDataTab()
	{
		...
		camDadStepperX = new PsychUINumericStepper(objX, objY, 50, cx, -10000, 10000, 2);
		camDadStepperY = new PsychUINumericStepper(objX + 80, objY, 50, cy, -10000, 10000, 2);
		...
		camGfStepperX = new PsychUINumericStepper(objX, objY, 50, cx, -10000, 10000, 2);
		camGfStepperY = new PsychUINumericStepper(objX + 80, objY, 50, cy, -10000, 10000, 2);
		...
		camBfStepperX = new PsychUINumericStepper(objX, objY, 50, cx, -10000, 10000, 2);
		camBfStepperY = new PsychUINumericStepper(objX + 80, objY, 50, cy, -10000, 10000, 2);
	...
	function addObjectTab()
	{
		...
		angleStepper = new PsychUINumericStepper(objX, objY, 10, 0, 0, 360, 2);
	...
	function addMetaTab()
	{
			...
			for (spr in stageSprites)
			{
				...
				switch(spr.type)
				{
					case 'sprite', 'animatedSprite':
						var preloadPath = normalizePreloadImagePath(spr.image);
						if(preloadPath != null && !lockedList.contains(preloadPath))
							lockedList.push(preloadPath);
				}
			}
		...
		{
			if(stageJson._editorMeta == null) stageJson._editorMeta = {boyfriend: 'bf', dad: 'dad', gf: 'gf'};
	...
	function updateSelectedUI()
	{
		...
		var displayX:Float = selected.x;
		var displayY:Float = selected.y;
	...
	function saveObjectsToJson()
	{
		...
		ensureStageCharacterObject('gf');
		ensureStageCharacterObject('dad');
		ensureStageCharacterObject('boyfriend');
	}

	function ensureStageCharacterObject(type:String)
	{
		if(stageJson.objects == null) stageJson.objects = [];

		for (object in stageJson.objects)
			if(object != null && Reflect.field(object, 'type') == type)
				return;

		stageJson.objects.push({type: type});
	}

	function prepareStageJsonForSave():Dynamic
	{
		saveObjectsToJson();
		ensureStageEditorMeta();
		normalizeStageJsonData();

		var generatedPreload = buildGeneratedPreload();
		if(generatedPreload != null)
			stageJson.preload = generatedPreload;
		else
			Reflect.deleteField(stageJson, 'preload');

		return stageJson;
	}

	function ensureStageEditorMeta()
	{
		if(stageJson._editorMeta == null)
			stageJson._editorMeta = {};

		if(boyfriend != null)
			Reflect.setField(stageJson._editorMeta, 'boyfriend', boyfriend.curCharacter);
		if(gf != null)
			Reflect.setField(stageJson._editorMeta, 'gf', gf.curCharacter);
		if(dad != null)
			Reflect.setField(stageJson._editorMeta, 'dad', dad.curCharacter);
	}

	function buildGeneratedPreload():Dynamic
	{
		var preload:Dynamic = {};
		...
		if(stageJson.preload != null)
		{
			for (asset in Reflect.fields(stageJson.preload))
			{
				var trimmedAsset = asset.trim();
				if(trimmedAsset.length < 1) continue;

				Reflect.setField(preload, trimmedAsset, parseLoadFilters(Reflect.field(stageJson.preload, asset)));
				found = true;
			}
		}
		...
		if(stageJson.objects != null)
		{
			for (object in stageJson.objects)
			{
				...
				if(addGeneratedPreload(preload, image, Reflect.field(object, 'filters')))
					found = true;
			}
		}
	...
	function addGeneratedPreload(preload:Dynamic, image:Dynamic, filters:Dynamic):Bool
	{
		var key = normalizePreloadImagePath(image);
		if(key == null) return false;

		var parsedFilters = parseLoadFilters(filters);
		var current:Dynamic = Reflect.field(preload, key);
		if(current != null)
			parsedFilters |= parseLoadFilters(current);

		Reflect.setField(preload, key, parsedFilters);
		return true;
	}

	static function normalizePreloadImagePath(image:Dynamic):String
	{
		if(image == null) return null;

		var path = Std.string(image).trim();
	...
	static function parseLoadFilters(filters:Dynamic):Int
	{
		if(filters == null) return LOW_QUALITY | HIGH_QUALITY;

		var parsed = Std.parseFloat(Std.string(filters));
		return Math.isNaN(parsed) ? (LOW_QUALITY | HIGH_QUALITY) : Std.int(parsed);
	}
