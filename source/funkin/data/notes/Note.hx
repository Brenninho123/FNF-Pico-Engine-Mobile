	...
	**/
	...
	public function reloadNote(texture:String = '', postfix:String = '') {
		...
		if(PlayState.isPixelStage) {
			...
			{
				frames = Paths.getSparrowAtlas(pixelAsset);
			}
			else
			{
			...
			frames = noteSkinAtlasExists(noteAsset) ? Paths.getSparrowAtlas(noteAsset) : null;
			if(frames == null)
				return;
	...
	public static function getNoteSkinConfig(texture:String):NoteSkinConfig
	{
		...
		var cachePath:String = hasStyleJson ? 'data/notestyles/$styleKey' : 'images/$texture';
	...
	static function parseNoteStyleUiAssets(config:NoteSkinConfig, assets:Dynamic)
	{
		var keys:Array<String> = [
			...
			'judgementMarvelous','judgementSick', 'judgementGood', 'judgementBad', 'judgementShit',
			'combo'
		];
		for (i in 0...10)
	...
	public static function noteSkinScale(config:NoteSkinConfig, assetType:String):Float
	{
		...
		return switch(assetType)
		{
			...
			case 'holdNote': config.noteScale;
			case 'noteStrumline': config.noteScale;
