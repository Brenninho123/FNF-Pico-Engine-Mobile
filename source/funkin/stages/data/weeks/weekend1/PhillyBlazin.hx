package funkin.stages.data.weeks.weekend1;

import funkin.stages.objects.ABotSpeaker;

import funkin.states.GameOverState;
import funkin.stages.data.weeks.weekend1.objects.characters.*;
...
class PhillyBlazin extends BaseStage
{
	...
	override function create()
	{
		...
		if(_song.gameOverSound == null || _song.gameOverSound.trim().length < 1) GameOverState.deathSoundName = 'fnf_loss_sfx-pico-gutpunch';
		if(_song.gameOverLoop == null || _song.gameOverLoop.trim().length < 1) GameOverState.loopSoundName = 'gameOver-pico';
		if(_song.gameOverEnd == null || _song.gameOverEnd.trim().length < 1) GameOverState.endSoundName = 'gameOverEnd-pico';
		if(_song.gameOverChar == null || _song.gameOverChar.trim().length < 1) GameOverState.characterName = 'pico-blazin';
		GameOverState.deathDelay = 0.15;
