...
class HoldNoteCover extends FlxSprite
{
	...
	public function spawnCover(strum:StrumNote, note:Note, phase:String, holdTime:Float = 0)
	{
		...
		var skin:String = Note.songArrowSkinForMustPress(note.mustPress);
		if(skin == null || skin.length < 1)
			skin = Note.defaultSongNoteStyle();

		var config:NoteSkinConfig = Note.getNoteSkinConfig(skin);
		...
		var assetPath:String = Note.resolveHoldNoteCoverAsset(config, note.noteData);
