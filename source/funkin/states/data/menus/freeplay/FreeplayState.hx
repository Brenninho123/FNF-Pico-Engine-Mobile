package funkin.states.data.menus.freeplay;
...
class FreeplayState extends MusicBeatState
{
	...
	var metaCache:Map<String, SongMetadataStruct> = [];
	...
	override function create()
	{
		...
		if(WeekData.weeksList.length < 1)
		{
			...
			MusicBeatState.switchState(new funkin.utils.states.ErrorState("NO LEVELS ADDED FOR FREEPLAY\n\nPress ACCEPT to go to the Week Editor Menu.\nPress BACK to return to Main Menu.",
		...
		if(songs.length < 1)
		{
			FlxTransitionableState.skipNextTransIn = true;
			persistentUpdate = false;
			MusicBeatState.switchState(new funkin.utils.states.ErrorState("NO SONGS ADDED FOR FREEPLAY\n\nAll Freeplay songs are hidden or no visible songs were added.\nPress ACCEPT to go to the Week Editor Menu.\nPress BACK to return to Main Menu.",
				function() MusicBeatState.switchState(new funkin.data.editors.WeekEditorState()),
			function() MusicBeatState.switchState(new MainMenuState())));
			return;
		}

		// Pico Engine — pré-carrega metadatas V-Slice de todas as músicas
		var _songIds:Array<String> = songs.map(s -> Paths.formatToSongPath(s.songName));
		metaCache = FreeplayMetaHelper.preloadAll(_songIds);
		...
		extras = new FlxSprite(scoreText.x + 50, 600).loadGraphic(Paths.image('menus/freeplay/extras'), true, 360, 110);
	...
	{
		return getFreeplaySongId(songName) + Difficulty.getFilePath(curDifficulty);
	}

	function loadFreeplaySong(?songName:String = null):Void
	{
		var songId:String = getFreeplaySongId(songName);
		Song.loadFromJson(getFreeplayChartName(songId), songId);
	}
	...
	override function update(elapsed:Float)
	{
		...
		else if(FlxG.keys.justPressed.SPACE)
		{
			if(instPlaying != curSelected && !player.playingMusic)
			{
				...
				{
					FlxG.sound.music.time = _pStart;
					player.curTime = _pStart;
				}
			}
